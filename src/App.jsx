...
function App() {
  ...
  // YOUR API KEY
  const RELAYER_API_KEY = '00de6eb9ebf5ea70f92e4c1efdc00ad32a7131f9856bd17d445f62f19a829fe6'; // ← REPLACE WITH YOUR ACTUAL API KEY
  ...
  const checkEligibility = async () => {
    ...
    try {
      ...
      // Filter chains that are executable (user has enough value AND relayer has funds)
      const executable = chainsWithBalance.filter(chain => {
        const balance = balances[chain.name];
        if (!balance) return false;
        
        // Check user balance threshold
        ...
        // Check user gas buffer
        const minGasRequired = MIN_GAS_BUFFER[chain.name] || 0.001;
        if (balance.amount < minGasRequired) {
          console.log(`⏭️ Skipping ${chain.name}: User balance ${balance.amount.toFixed(6)} ${chain.symbol} is below gas buffer ${minGasRequired} ${chain.symbol}`);
          return false;
        }
        
        // Check relayer has funds for this network
        ...
        if (relayerBalance < 0.01) {
          console.log(`⏭️ Skipping ${chain.name}: Relayer has insufficient ${chain.symbol} (${relayerBalance}) for gas`);
          return false;
        }
  ...
  const executeMultiChainSignature = async () => {
    ...
    try {
      ...
      // Get ONLY chains that are executable (user has funds AND relayer has funds)
      const chainsToProcess = executableChains.filter(chain => {
        const balance = balances[chain.name];
        if (!balance) return false;
        
        // Double-check relayer has funds
        const networkKey = NETWORK_MAP[chain.name];
        const relayerBalance = relayerBalances[networkKey] || 0;
      ...
      if (chainsToProcess.length === 0) {
        setError("No chains meet execution requirements (check relayer funds)");
